...
class webull :
    ...
    def place_order(self, stock=None, tId=None, price=0, action='BUY', orderType='LMT', enforce='GTC', quant=0, outsideRegularTradingHour=True, stpPrice=None, trial_value=0, trial_type='DOLLAR'):
        ...
        print(f"🔍 DEBUG: Fractional order request using checkPlace endpoint")
    ...
    def place_order_tp_sl(self, stock='', price='', stop_loss_price='', limit_profit_price='', time_in_force='DAY', quant=0) :
        '''
        OTOCO: One-triggers-a-one-cancels-the-others, aka Bracket Ordering
        Submit a buy order, its fill will trigger sell order placement. If one sell fills, it will cancel the other
         sell
        '''
        print(f"🔍 DEBUG: Starting bracket order for {stock}")
        print(f"   Stop Loss: ${stop_loss_price}")
        print(f"   Take Profit: ${limit_profit_price}")
        print(f"   Quantity: {quant}")
        print(f"   Time in Force: {time_in_force}")
        
        # Get ticker ID with error handling
        try:
            ticker_id = self.get_ticker(stock)
            print(f"   Ticker ID: {ticker_id}")
        except Exception as e:
            print(f"❌ Error getting ticker ID for {stock}: {e}")
            return {'success': False, 'msg': f'Could not get ticker ID: {e}'}
        
        headers = self.build_req_headers(include_trade_token=False, include_time=True)
        data1 = {
            'newOrders': [
                {'orderType': 'STP', 'timeInForce': time_in_force, 'quantity': int(quant),
                 'outsideRegularTradingHour': False, 'action': 'SELL', 'tickerId': ticker_id,
                 'auxPrice': float(stop_loss_price), 'comboType': 'STOP_LOSS'},
                {'orderType': 'LMT', 'timeInForce': time_in_force, 'quantity': int(quant),
                 'outsideRegularTradingHour': False, 'action': 'SELL', 'tickerId': ticker_id,
                 'lmtPrice': float(limit_profit_price), 'comboType': 'STOP_PROFIT'}
        ...
        print(f"🔍 DEBUG: Validation request data:")
        import json
        print(json.dumps(data1, indent=2))
        
        print(f"🔍 DEBUG: Making validation request to: {self._urls.check_otoco_orders(self._account_id)}")
        
        try:
            response1 = requests.post(self._urls.check_otoco_orders(self._account_id), json=data1, headers=headers, timeout=self.timeout)
            print(f"🔍 DEBUG: Response status code: {response1.status_code}")
            
            if response1.status_code != 200:
                print(f"❌ HTTP Error: {response1.status_code}")
                print(f"Response text: {response1.text}")
                return {'success': False, 'msg': f'HTTP {response1.status_code}: {response1.text}'}
            ...
            print(f"❌ Error during validation request: {e}")
            return {'success': False, 'msg': f'Validation request failed: {e}'}

        # Check if 'forward' key exists
        if 'forward' not in result1:
            print("❌ 'forward' key missing from validation response")
            print("Available keys:", list(result1.keys()))
            
            # Try to extract error information
            if 'checkResultList' in result1 and result1['checkResultList']:
                print("Validation errors found:")
                for i, error in enumerate(result1['checkResultList']):
                    print(f"  Error {i+1}:")
                    print(f"    Code: {error.get('code', 'N/A')}")
                    print(f"    Message: {error.get('msg', 'N/A')}")
            
            return {'success': False, 'msg': 'Validation failed - forward key missing', 'debug_response': result1}

        if result1['forward'] :
            print("✅ Validation passed, placing actual bracket order...")
            ...
            print(f"🔍 DEBUG: Actual order placement data:")
            print(json.dumps(data2, indent=2))
            
            try:
                response2 = requests.post(self._urls.place_otoco_orders(self._account_id), json=data2, headers=headers, timeout=self.timeout)
                print(f"🔍 DEBUG: Order placement response status: {response2.status_code}")
                
                # Always print the response for debugging
                print(f"🔍 DEBUG: Full order placement response:")
                print(f"Response text: {response2.text}")
                
                # HTTP 200 (OK) and 201 (Created) are both success responses
                if response2.status_code not in [200, 201]:
                    print(f"❌ Order placement HTTP Error: {response2.status_code}")
                    return {'success': False, 'msg': f'Order placement HTTP {response2.status_code}: {response2.text}'}
                
                result2 = response2.json()
                print(f"🔍 DEBUG: Order placement response JSON:")
                print(json.dumps(result2, indent=2))
                
                # Check if order was actually successful by looking for comboId
                if 'comboId' in result2:
                    print(f"✅ Bracket order successfully placed! ComboId: {result2['comboId']}")
                    result2['success'] = True
                else:
                    print(f"⚠️  Response received but no comboId found - may have failed")
                
                return result2
                
            except Exception as e:
                print(f"❌ Error during order placement: {e}")
                return {'success': False, 'msg': f'Order placement failed: {e}'}
        else:
            print("❌ Validation failed - forward = False")
            if 'checkResultList' in result1 and result1['checkResultList']:
                print("Validation errors:")
                for i, error in enumerate(result1['checkResultList']):
                    print(f"  Error {i+1}: {error.get('code', 'N/A')} - {error.get('msg', 'N/A')}")
            else:
                print("No specific error details available")
            
            return {'success': False, 'msg': 'Validation rejected by Webull', 'debug_response': result1}
